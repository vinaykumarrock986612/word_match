...
class GridViewModel {
  ...
  List<Map<String, int>> horizontalMatchAlgo() {
    int row = CreateGrid.grid.length;
    int column = CreateGrid.grid[0].length;

    int? horizontalMatchIndexStart;
    int? horizontalMatchIndexEnd;
    int? horizontalMatchLine;

    List<Map<String, int>> horizontalMatchIndex = [];

    List<String> horizontalMatchList = [];
    String word = "";
    ...
    for (int rowIndex = 0; rowIndex < row; rowIndex++) {
      ...
      for (int columnIndex = 0; columnIndex < column;) {
        ...
        if (checkItemExist(item: CreateGrid.grid[rowIndex][columnIndex])) {
          word = CreateGrid.grid[rowIndex][columnIndex];
          for (int i = columnIndex + 1; i < column; i++) {
            if (checkItemExist(item: CreateGrid.grid[rowIndex][i])) {
              word = word + CreateGrid.grid[rowIndex][i];
              for (int wordIndex = 0; wordIndex < word.length; wordIndex++) {
                String subWord = word.substring(wordIndex);
                if (wordList.contains(subWord.toLowerCase())) {
                  horizontalMatchList.add(word);
                  horizontalMatchIndexStart = columnIndex + wordIndex;
                  horizontalMatchIndexEnd = i;
                  horizontalMatchLine = rowIndex;

                  horizontalMatchIndex.add({
    ...
    // print(horizontalMatchList);
    return horizontalMatchIndex;
  }

  List<Map<String, int>> verticalMatchAlgo() {
    int row = CreateGrid.grid.length;
    int column = CreateGrid.grid[0].length;
    ...
    List<Map<String, int>> verticalMatchIndex = [];
      ...
      for (int rowIndex = 0; rowIndex < row;) {
        ...
        if (checkItemExist(item: CreateGrid.grid[rowIndex][columnIndex])) {
          ...
          for (int i = rowIndex + 1; i < row; i++) {
            if (checkItemExist(item: CreateGrid.grid[i][columnIndex])) {
              word = word + CreateGrid.grid[i][columnIndex];

              for (int wordIndex = 0; wordIndex < word.length; wordIndex++) {
                String subWord = word.substring(wordIndex);
                if (wordList.contains(subWord.toLowerCase())) {
                  verticalMatchList.add(word);
                  verticalMatchIndexStart = rowIndex + wordIndex;
                  verticalMatchIndexEnd = i;
                  verticalMatchLine = columnIndex;

                  verticalMatchIndex.add({
  ...
  bool horizontalMatch({int? rowIndex, int? columnIndex}) {
    List<Map<String, int>> horizontalMatchList = horizontalMatchAlgo();
    if (GridViewModel.gridSearchController.text != "") {
      if (horizontalMatchList != null) {
        for (int i = 0; i < horizontalMatchList.length; i++) {
          if (horizontalMatchList[i]["horizontalMatchLine"] == rowIndex) {
            int? start = horizontalMatchList[i]["horizontalMatchIndexStart"];
            int? end = horizontalMatchList[i]["horizontalMatchIndexEnd"];
  ...
  bool verticalMatch({int? rowIndex, int? columnIndex}) {
    List<Map<String, int>> verticalMatchList = verticalMatchAlgo();
    if (GridViewModel.gridSearchController.text != "") {
      if (verticalMatchList != null) {
        for (int i = 0; i < verticalMatchList.length; i++) {
          if (verticalMatchList[i]["verticalMatchLine"] == columnIndex) {
            int? start = verticalMatchList[i]["verticalMatchIndexStart"];
            int? end = verticalMatchList[i]["verticalMatchIndexEnd"];
            if (start != null && end != null) {
